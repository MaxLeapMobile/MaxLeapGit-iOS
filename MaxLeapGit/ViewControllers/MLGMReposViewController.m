    ...
    if (!_tableView) {
        ...
        [self.tableView addPullToRefreshWithActionHandler:^{
            ...
            if (self.type == MLGMReposControllerTypeRepos) {
                [kWebService fetchPublicReposForUserName:weakSelf.ownerName fromPage:page completion:^(NSArray *repos, BOOL isReachEnd, NSError *error) {
            ...
            if (self.type == MLGMReposControllerTypeStars) {
                [kWebService fetchStarredReposForUserName:weakSelf.ownerName fromPage:page completion:^(NSArray *repos, BOOL isReachEnd, NSError *error) {
                    execute_after_main_queue(0.2, ^{
                        [weakSelf.tableView.pullToRefreshView stopAnimating];
                    });
                    weakSelf.tableView.showsInfiniteScrolling = !isReachEnd;
        ...
        [self.tableView addInfiniteScrollingWithActionHandler:^{
            if (self.type == MLGMReposControllerTypeRepos) {
                [kWebService fetchPublicReposForUserName:weakSelf.ownerName fromPage:page + 1 completion:^(NSArray *repos, BOOL isReachEnd, NSError *error) {
            ...
            if (self.type == MLGMReposControllerTypeStars) {
                [kWebService fetchStarredReposForUserName:weakSelf.ownerName fromPage:page + 1 completion:^(NSArray *repos, BOOL isReachEnd, NSError *error) {
                    [weakSelf.tableView.infiniteScrollingView stopAnimating];
                    weakSelf.tableView.showsInfiniteScrolling = !isReachEnd;
                    if (!error) {
                        page++;
